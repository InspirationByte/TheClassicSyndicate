...
MISSION.InitSceneryObject = function()
	...
	local defData = create_section {
		...
		velocity = 4.0,
		dir_angles = {65, 0, 0},		-- direction angle of emitting with specified velocity
	...
	emitterObj:SetParent(speedboatObject)
	emitterObj:SetLocalOrigin(vec3(-0.5,-0.5,-4.0))
	emitterObj:SetLocalAngles(vec3(0, 180, 0))
	emitterObj:Spawn()
	
	local emitterObjL = objects:CreateObject("speedboat_splashes")
	emitterObjL:SetParent(speedboatObject)
	emitterObjL:SetLocalOrigin(vec3(0.5,-0.8,2.0))
	emitterObjL:SetLocalAngles(vec3(0,90,0))
	emitterObjL:Spawn()
	
	local emitterObjR = objects:CreateObject("speedboat_splashes")
	emitterObjR:SetParent(speedboatObject)
	emitterObjR:SetLocalOrigin(vec3(0,-0.8,2.0))
	emitterObjR:SetLocalAngles(vec3(0,-90,0))
	emitterObjR:Spawn()
...
MISSION.Init = function()		
	...
	tempCar:Spawn()
	gameses:SetPlayerCar(tempCar)
	gameses:SetViewObject( playerCar )
...
function MISSION.Phase1Start()
	...
	local activeLife = playerCar:AddComponent(ActiveLifeAIComponent)
	activeLife:SetPersonalityType("racer")
	activeLife:SetTargetPosition(MISSION.Data.targetPosition)
